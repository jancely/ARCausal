...
class FullAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask, tau=None, delta=None):
        ...
        scores = torch.where(torch.all(scores, dim=-1, keepdim=True), torch.zeros_like(scores), scores)
        #print(scores.shape, attn_mask.shape)
        #trans = (0.5 * torch.log(attn_mask / scores))
        #trans = torch.where(trans > 0., trans, 0.).bool()
        #trans = torch.softmax(torch.nan_to_num(trans, nan=1e-5), dim=-1)
        #trans = torch.clamp(trans, min=0.)
        #print('trans', trans)
        assert not torch.isnan(scores).any(), 'Fins nan scores'
        ...
        #scores = scores.masked_fill_(trans, -np.inf)
        #print('attn mask', attn_mask.shape, scores.shape)
        #print('attn mask', attn_mask)
        #print('scores', scores)
        #assert not torch.isnan(attn_mask).any(), 'Find nan mask'
        #assert not torch.isnan(scores).any(), 'Find nan score'
        #scores = torch.mul(scores, attn_mask.unsqueeze(1))
        #print('scores', scores)
        #key = scale * scores
        A = self.dropout(torch.softmax(scale * scores, dim=-1))
        assert not torch.isnan(A).any(), 'Find nan key'
            #break
        #A = self.dropout(key)
...
# Code implementation from https://github.com/jancely/
